...
**OBRIGATÓRIO**: Toda comunicação com o usuário durante a execução desta skill deve ser em **português (pt_BR)**. Isso inclui:
- Perguntas e confirmações
- Resumos das mudanças
- Descrições dos commits
- Mensagens de erro ou aviso

As mensagens de commit em si devem seguir o padrão convencional do projeto (detectado no histórico git), mas todo texto dirigido ao usuário deve estar em português.
